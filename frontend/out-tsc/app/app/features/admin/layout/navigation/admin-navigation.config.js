import { LucideBedDouble, LucideBookOpen, LucideBookOpenCheck, LucideBuilding2, LucideCalendarCheck, LucideClipboardCheck, LucideClipboardList, LucideDatabase, LucideDoorOpen, LucideFilePenLine, LucideGlobe, LucideGraduationCap, LucideHouse, LucideLandmark, LucideLayers, LucideLayoutDashboard, LucideMap, LucideMapPinned, LucideReceiptIndianRupee, LucideSettings2, LucideTableProperties, LucideTags, LucideTrash2, LucideUniversity, LucideUserPlus, LucideUsersRound, LucideWalletCards, } from '@lucide/angular';
...
const exact = (path) => new RegExp(`^${escapeRegExp(path)}/?$`);
const branch = (path) => new RegExp(`^${escapeRegExp(path)}(?:/|$)`);
...
export function adminNavigationIcon(icon) {
    return ADMIN_NAVIGATION_ICONS[icon];
}
export function matchesAdminRoute(patterns, url) {
    const path = normaliseAdminPath(url);
    return patterns?.some((pattern) => pattern.test(path)) ?? false;
}
export function findAdminNavigationTrail(url) {
    const path = normaliseAdminPath(url);
    for (const section of ADMIN_NAVIGATION) {
        if (!matchesAdminRoute(section.activeWhen, path))
            continue;
        if (!section.children)
            return { section };
        for (const entry of section.children) {
            if (entry.children) {
                ...
                if (link)
                    return { section, cluster: entry, link };
            }
...
export function resolveAdminPageContext(url, routeTitle = '') {
    const path = normaliseAdminPath(url);
    ...
    if (trail?.cluster && trail.cluster.label !== title)
        breadcrumbs.push(trail.cluster.label);
    if (trail?.link && trail.link.label !== title && trail.link.label !== trail.cluster?.label) {
        breadcrumbs.push(trail.link.label);
    }
    return { title, breadcrumbs };
}
function masterLink(slug, label, icon) {
    return {
        id: `master-${slug}`,
        label,
...
function resolveMasterDataTitle(path) {
    const match = path.match(/^\/admin\/master-data\/([^/]+)(?:\/([^/]+))?(?:\/([^/]+))?\/?$/);
    if (!match || match[1] === 'hostel')
        return '';
...
function humaniseSlug(slug) {
    ...
    return (knownLabels[slug] || slug.replaceAll('-', ' ').replace(/^./, (letter) => letter.toUpperCase()));
}
function normaliseAdminPath(url) {

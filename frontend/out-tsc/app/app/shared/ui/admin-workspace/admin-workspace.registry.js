...
export function resolveAdminWorkspace(url) {
    const path = url.split(/[?#]/)[0].replace(/\/+$/, '');
    if (!path.startsWith('/admin/') || path.startsWith('/admin/academics'))
        return null;
    const link = (label, route, active = path === route) => ({
        label,
        route,
        active,
    });
    const workspace = (module, title, illustration, links = []) => ({ module, title, illustration, links });
    ...
    const fee = path.match(/^\/admin\/fees\/(books|heads|hostel-fees|course-fees|scholarships)(?:\/|$)/);
    if (fee) {
        const titles = {
            books: 'Fee books',
            heads: 'Fee heads',
        ...
        return workspace('Finance', titles[fee[1]], fee[1] === 'scholarships' ? 'scholarshipEmpty' : 'feeConfiguration', links);
    }
    ...
    if (/^\/admin\/fees\/(overview|schedules|progression)$/.test(path)) {
        return workspace('Finance', path.endsWith('overview')
            ? 'Finance operations'
            : path.endsWith('schedules')
                ? 'Fee publication'
                : 'Fee progression', path.endsWith('overview') ? 'feeWallet' : 'taskSchedule', [
    ...
    if (student && student[1] !== 'forms') {
        const id = student[1];
        return workspace(student[2] === 'edit' ? 'Students' : 'Finance', 'Student workspace', student[2] === 'scholarships'
            ? 'scholarshipEmpty'
    ...
    if (path.startsWith('/admin/master-data/hostel/')) {
        ...
        return workspace('Operations', 'Hostel management', 'roomUnavailable', pages.map(([slug, label]) => link(label, `/admin/master-data/hostel/${slug}`)));
    }
    const master = path.match(/^\/admin\/master-data\/([^/]+)/);
    if (master)
        return workspace('Configuration', 'Master data', 'dataConfiguration', directory(`/admin/master-data/${master[1]}`));
    if (path === '/admin/form-builder' ||
        ...
        if (/\/forms\/[^/]+\/edit$/.test(path))
            links.push(link('Edit form', path));
        if (/\/forms\/[^/]+\/applications$/.test(path))
            links.push(link('Form applications', path));
    ...
    if (path.startsWith('/admin/database/') ||
        ...
        if (path !== '/admin/database/students')
            links.push(link(path.endsWith('/edit') ? 'Edit record' : path.endsWith('/fill') ? 'Fill form' : 'Records', path));
    ...
    if (path.startsWith('/admin/admissions/') || path === '/admin/delete-admissions') {
        return workspace('Students', 'Student management', path.endsWith('promotions') ? 'taskSchedule' : 'addStudent', [
